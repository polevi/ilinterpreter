...
namespace ILDisassembler
{
    public class MethodBodyReader
    {
        ...
        private void ConstructInstructions(Module module, LocalScope scope)
        {
            byte[] il = this.il;
            int position = 0;
        ...
        /// </param>
        public MethodBodyReader(MethodInfo mi, LocalScope scope)
        {
            this.mi = mi;
            if (mi.GetMethodBody() != null)
            {
                il = mi.GetMethodBody().GetILAsByteArray();
                ConstructInstructions(mi.Module, scope);
            }
        }
    }
}
